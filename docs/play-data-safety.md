...
- [x] **Files and docs** -> Collected: **No**
  - The app writes only to its own private app directory (/data/data/io.github.zansuken.cairn/files/cairn.sqlite and /data/data/io.github.zansuken.cairn/files/cairn_interceptions.jsonl). These are internal app storage, not user documents, and the app has no INTERNET permission to transmit them.
...
- [x] **App activity / App info and performance** -> Collected: **No**
  - The app uses the Android UsageStatsManager API (PACKAGE_USAGE_STATS permission) to detect when tracked apps come to the foreground, and it derives streaks and interception-event outcomes from that. All computation and storage is on-device only (SQLite tables DayRecords, AppStreakStates, MetaStates, InterceptionEvents, plus the native JSONL journal at /data/data/io.github.zansuken.cairn/files/cairn_interceptions.jsonl and SharedPreferences at /data/data/io.github.zansuken.cairn/shared_prefs/cairn_worker.xml). None of this data is transmitted off the device, so under Play's definition it is not "collected." There is no analytics, crash reporting, or telemetry SDK. The analyticsOptIn field in the database schema (default false) has no associated collection logic and is not used by the app.
...
- [x] **Do you provide a way for users to request that their data be deleted?**
  - All user data is stored locally on the device only in /data/data/io.github.zansuken.cairn/files (SQLite database cairn.sqlite and JSONL journal cairn_interceptions.jsonl) and /data/data/io.github.zansuken.cairn/shared_prefs (Android SharedPreferences cairn_worker.xml). Users can delete all app data at any time by uninstalling the app or by clearing the app's storage in Android system settings. No server-side data exists, so there is no remote data to delete and no deletion request mechanism is needed.
...
- All data lives in the app's private storage directories: /data/data/io.github.zansuken.cairn/files (SQLite) and /data/data/io.github.zansuken.cairn/shared_prefs (Android SharedPreferences).
